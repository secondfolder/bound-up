...
/**
 * This app's own columns for a registered passkey.
 *
 * A sidecar table rather than columns on `passkey`, because `schema/auth.ts` is
 * generated by `npm run auth:schema` and anything hand-added there is lost on
 * the next regeneration (invariant 7). So this is where anything the app wants
 * to remember about a credential goes, and adding a second fact later is a
 * column here rather than another table.
 *
 * One row per passkey, created lazily: a passkey with nothing worth recording
 * has no row at all, and every column is therefore nullable.
 */
export const passkeyDetails = sqliteTable('passkey_details', {
	id: text('id')
		.primaryKey()
		.$defaultFn(() => crypto.randomUUID()),
	// Unique, and cascading: these facts are about one credential, and leaving
	// them behind would let a later passkey inherit a stale answer if ids were
	// ever reused.
	passkeyId: text('passkey_id')
		.notNull()
		.unique()
		.references(() => passkey.id, { onDelete: 'cascade' }),
	// Denormalised so every read here can be scoped to its owner without a join
	// through the generated table.
	userId: text('user_id')
		.notNull()
		.references(() => user.id, { onDelete: 'cascade' }),
	/**
	 * Whether this credential can unlock message keys.
	 *
	 * The result of a **real PRF evaluation** against it, never the
	 * `prf.enabled` flag the registration response carries. Those disagree in
	 * both directions in the field: Samsung Pass and KeePassXC report
	 * `enabled: false` at creation and then return PRF output perfectly well at
	 * assertion, and Microsoft Password Manager does the reverse. Storing the
	 * flag would write off working authenticators and vouch for broken ones.
	 * See docs/passkeys.md.
	 *
	 * NULL is a real third state and must not be rendered as a warning: it means
	 * nothing has ever been proved about this passkey, which is true of every
	 * one registered before the check existed.
	 */
	prfStatus: text('prf_status').$type<PasskeyPrfStatusValue>(),
	/**
	 * The AAGUID as seen at registration, so the provider can be named later.
	 *
	 * Better Auth stores this on the passkey row too. It is copied because Apple
	 * zeroes it under `attestation: "none"` and a future change to that option
	 * would otherwise make old rows unexplainable.
	 */
	aaguid: text('aaguid'),
	...timestamps
});
...
export type PasskeyDetails = typeof passkeyDetails.$inferSelect;
export type NewPasskeyDetails = typeof passkeyDetails.$inferInsert;
