...
describe('TimezoneWarning', () => {
	it('shows when the device timezone differs from the account timezone', () => {
	...
	it('stays hidden when the device timezone matches', () => {
	...
	it('dismisses and caches the dismissal for the current device timezone', async () => {
		const { container } = render(TimezoneWarning, { user });
		const [, dismiss] = container.querySelectorAll('wa-button');
		if (!(dismiss instanceof HTMLElement)) {
			throw new Error('missing dismiss button');
		}
		await fireEvent.click(dismiss);

		expect(globalThis.localStorage.getItem('bound-up:timezone-banner:u1')).toBe('America/New_York');
		expect(screen.queryByText('This device is in a different timezone')).not.toBeInTheDocument();
	});

	it('honours a cached dismissal only for the same device timezone', () => {
		globalThis.localStorage.setItem('bound-up:timezone-banner:u1', 'America/New_York');
