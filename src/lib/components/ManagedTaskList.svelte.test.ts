...
describe('ManagedTaskList', () => {
	it('shows a plus sign for multi-credit task rewards', () => {
	...
	it('shows a plus sign for single-credit task rewards', () => {
		render(ManagedTaskList, {
	...
	it('omits the credit label for zero-credit tasks', () => {
		render(ManagedTaskList, {
	...
	it('omits the generic unavailable copy for partnership tasks created by the current user', () => {
		render(ManagedTaskList, {
	...
	it('does not show ready-to-complete or authorship helper text', () => {
		render(ManagedTaskList, {
	...
	it('does not render an edit button for partnership tasks created by the other partner', () => {
	...
	it('renders the timezone note directly after the schedule text', () => {
		const { container } = render(ManagedTaskList, {
			tasks: [
				partnershipTask({
		...
		if (!(timezone instanceof HTMLElement)) {
			throw new Error('missing timezone note');
		}
	...
	it('renders task actions in a separate right-hand column', () => {
		const { container } = render(ManagedTaskList, {
			tasks: [task()],
			emptyMessage: 'No tasks',
		...
		if (!(actionsColumn instanceof HTMLElement)) {
			throw new Error('missing actions column');
		}
	...
	it('renders inactive task credits with muted styling', () => {
		const { container } = render(ManagedTaskList, {
		...
		if (!(credits instanceof HTMLElement)) {
			throw new Error('missing action credits');
		}
