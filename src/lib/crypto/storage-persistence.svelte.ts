 ...
 * something to refuse. So nothing asks on its own any more: being sent back
 * to sign in because the browser lost its key makes an explanation due,
 * `StoragePersistenceDialog` shows it, and only its OK button asks the browser.
 * The Security page can ask again.
 *
 * Why that sign-in rather than any: it is the moment the cost of eviction has
 * just been felt — the user had to sign in again because storage was empty —
 * and not the moment they first signed in, when nothing had gone wrong yet.
 */
 ...
 * dismissal: closing the dialog asks nothing, so it comes back the next time
 * the browser loses the key.
 */
 ...
 * the user signs out does not reopen the dialog for a keyring that is gone.
 */
let generation = 0;
...
function rememberAsked(): void {
	try {
		...
		// Survivable: the dialog comes back the next time it is due.
	}
}
...
/**
 * Called after a sign-in the browser's lost key forced. Makes the explanation
 * due, unless there is nothing worth explaining.
 *
...
/** The dialog closed without OK: ask nothing, and offer again next time. */
 ...
 *
 * Also what the Security page calls directly, for a dismissed dialog or a
 * browser that said no the first time. Same user-activation rule as above.
 */
...
/** Test seam: forget whether anything is due. */
